...
impl std::str::FromStr for Barcode {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        let leading_nibble = u8::from_str_radix(&s[0..1], 16).map_err(|_| InvalidBarcodeError(s.into()))?;
...
impl std::fmt::Display for Barcode {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        ...
        let nibbles = [
            bytes[3] & 0xf,
            bytes[4] >> 4, bytes[4] & 0xf,
            bytes[5] >> 4, bytes[5] & 0xf,
            bytes[6] >> 4, bytes[6] & 0xf,
            bytes[7] >> 4, bytes[7] & 0xf,
        ];
...
mod tests {
    ...
    #[test]
    fn crc() {
        assert_eq!(super::crc(pv::LongAddress([0x04, 0xC0, 0x5B, 0x40, 0x00, 0x9A, 0x57, 0xA2])), 'L' as u8);
        assert_eq!(super::crc(pv::LongAddress([0x04, 0xC0, 0x5B, 0x40, 0x00, 0x79, 0xAC, 0x16])), 'V' as u8);
        assert_eq!(super::crc(pv::LongAddress([0x04, 0xC0, 0x5B, 0x40, 0x00, 0x79, 0xAB, 0x99])), 'W' as u8);
    }
